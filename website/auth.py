    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                flash('Logged in succesfullt!', category='success')
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Passwords must match each other.', category='error')
            ...
            db.session.commit()
            flash('Konto zostało utworzone pomyślnie! Możesz się teraz zalogować i korzystać z wszystkich usług.', category='success')
